...
export class ConversionController {
  ...
  private checkParams(queryParams: queryParams) {
    ...
    if ( !currencyCodes.some(item => queryParams.from.includes(item)) ) throw new HttpException(`Currency code ${queryParams.from} not found. Try access /currency-odes for available currency codes.`, HttpStatus.BAD_REQUEST);

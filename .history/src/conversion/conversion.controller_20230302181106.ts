...
export class ConversionController {
  ...
  private checkParams(queryParams: queryParams) {
    ...
    if ( !currencyCodes.some(item => queryParams.from.includes(item)) ) throw new HttpException(`Currency code ${queryParams.from} not found. Try access /currency-codes for available currency codes.`, HttpStatus.BAD_REQUEST);
    if ( !currencyCodes.some(item => queryParams.to.includes(item)) ) throw new HttpException(`Currency code ${queryParams.to} not found. Try access /currency-odes for available currency codes.`, HttpStatus.BAD_REQUEST);
